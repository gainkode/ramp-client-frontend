...
export class ProfileInfoComponent implements OnInit, OnDestroy {
      ...
      this.pSettingsSubscription = currencyData.valueChanges.subscribe(
        ({ data }) => {
          ...
          if (currencySettings !== null) {
            ...
            if (itemCount > 0) {
              this.currencies = currencySettings.list?.filter((x) => x.fiat).map(
                (val) =>
                  <CommonTargetValue>{
                    id: val.symbol,
                    title: val.name,
  ...
  private loadForm(): void {
    this.userForm.get('firstName')?.setValue(this.pUser?.firstName);
    this.userForm.get('lastName')?.setValue(this.pUser?.lastName);
    const selectedCountry = getCountryByCode3(
      this.pUser?.countryCode3 as string
    );
    if (selectedCountry) {
      this.userForm.get('country')?.setValue(selectedCountry.name);
    }
    const userItem = new UserItem(this.pUser);
    this.userForm.get('email')?.setValue(this.pUser?.email);
    this.userForm.get('phone')?.setValue(this.pUser?.phone);
    this.userForm.get('currency')?.setValue(this.pUser?.defaultFiatCurrency);
    this.accountId = this.pUser?.userId;
    this.email = this.pUser?.email;
    this.address = userItem.address;
    this.countryName = getCountryByCode3(this.pUser?.countryCode3 as string)?.name as string;
  }
  ...
  kycCompleted(): boolean {
    ...
    if (this.pUser) {
      result =
        this.pUser.kycStatus === 'completed' && this.pUser.kycValid === true;
    }
    return result;
  }
