...
import { routes } from 'personal/personal-routing.module';
...
export class MerchantGuard {
	...
	async canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Promise<boolean> {
		...
		// If user mode is not Merchant we cant allow him to open his account
		...
		if (!isAuthentificatedUser) {
			...
			if (route.data.authRoute) {
				...
				await this.router.navigateByUrl(`/merchant/${mainAuthentificatedRoute.path}`);
				return false;
			}
		}
